...
func parseSubscription(link string) ([]config.Node, error) {
    ...
    if strings.HasPrefix(link, "socks://") || strings.HasPrefix(link, "http://") || strings.HasPrefix(link, "https://") {
...
func parseVMess(link string) (config.Node, error) {
    ...
    if cfg.Net != "" { settings["network"] = cfg.Net }
    if cfg.Host != "" { settings["server_name"] = cfg.Host }
    if cfg.Path != "" { settings["path"] = cfg.Path }
    if strings.EqualFold(cfg.TLS, "tls") { settings["tls"] = "true" }
    return config.Node{
...
func parseVLess(link string) (config.Node, error) {
    ...
    vals := u.Query()
    if vals.Get("type") != "" { settings["network"] = vals.Get("type") }
    if vals.Get("host") != "" { settings["server_name"] = vals.Get("host") }
    if vals.Get("path") != "" { settings["path"] = vals.Get("path") }
    if vals.Get("security") == "tls" { settings["tls"] = "true" }
    name := u.Fragment
    if name == "" { name = "vless-" + u.Hostname() }
    return config.Node{
        ID:       "vless-" + u.Host,
        Name:     name,
        Protocol: "vless",
...
func parseTrojan(link string) (config.Node, error) {
    ...
    if password == "" { password = u.User.Username() }
    ...
    if vals.Get("security") == "tls" { settings["tls"] = "true" }
    name := u.Fragment
    if name == "" { name = "trojan-" + u.Hostname() }
    return config.Node{
        ID:       "trojan-" + u.Host,
        Name:     name,
        Protocol: "trojan",
...
func parseHTTPStyle(link string) (config.Node, error) {
    ...
    protocol := "http"
    if strings.HasPrefix(link, "socks://") { protocol = "socks" }
    settings := map[string]string{}
    if u.User != nil {
        settings["username"] = u.User.Username()
        if pw, ok := u.User.Password(); ok { settings["password"] = pw }
    }
    name := u.Fragment
    if name == "" { name = protocol + "-" + u.Hostname() }
    return config.Node{
        ID:       protocol + "-" + u.Host,
        Name:     name,
        Protocol: protocol,
...
func parsePort(s string) (uint16, error) {
    ...
    if err != nil || p == 0 { return 0, fmt.Errorf("无效端口: %s", s) }
    return p, nil
}
